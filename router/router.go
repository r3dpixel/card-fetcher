...
func EnvConfigured() *Router {
	...
	// Create builders with environment variables (PYGMALION_USERNAME, PYGMALION_PASSWORD, JANNY_CF_COOKIE, JANNY_USER_AGENT)
...
// AutoFetchConfigured creates a new router with default builders configured for auto-fetching cookies using chromedp
func AutoFetchConfigured(chromeCustomPath ...string) *Router {
	// Create a new router with default options
	r := New(
		reqx.Options{
			RetryCount:        4,
	...
	if len(chromeCustomPath) > 0 {
		customPath = chromeCustomPath[0]
	}

	// Create builders with auto-fetching cookies using chromedp
	// Env variables are used for PYGMALION credentials (PYGMALION_USERNAME, PYGMALION_PASSWORD)
	builders := impl.DefaultBuilders(impl.BuilderOptions{
		PygmalionIdentityReader: cred.NewManager("pygmalion", cred.Env),
		JannyChromeConfig: func() impl.JannyChromeConfig {
			return impl.JannyChromeConfig{
				Path:             customPath,
				AutoFetchCookies: true,
			}
		},
	})
...
// IsChromeOK checks if Chrome is available and can be used for auto-fetching cookies
func IsChromeOK(chromeConfig func() impl.JannyChromeConfig) bool {
	// If chromeConfig is not provided, return false
	if chromeConfig == nil {
		return false
	}
	// Get chrome config
	...
	// If auto-fetching cookies is disabled, return false
	if !config.AutoFetchCookies {
		return false
	}

	// Create default flags
	opts := chromex.DefaultFlags()

	// Add headless flag
	opts = append(opts, chromedp.Headless)

	// Add a custom Chrome path if provided
	if !stringsx.IsBlank(config.Path) {
		opts = append(opts, chromedp.ExecPath(config.Path))
	}

	// Try to create an exec allocator (this will fail if Chrome is not available)
	allocCtx, cancel := chromedp.NewExecAllocator(context.Background(), opts...)
	defer cancel()

	// Try to create a context with a timeout
	...
	// Try to navigate to verify Chrome is working
