...
var afTestRouter = router.AutoFetchConfigured()
...
func FetchAndAssert(t *testing.T, url string) *CharacterAssertion {
	return FetchAndAssertWithRouter(t, testRouter, url)
}

func FetchAndAssertWithRouter(t *testing.T, router *router.Router, url string) *CharacterAssertion {
	fetcherTask, ok := router.TaskOf(url)
...
func (ca *CharacterAssertion) AssertPygmalionCredentials() *CharacterAssertion {
	...
	for _, envVar := range requiredEnvVars {
		if stringsx.IsBlank(os.Getenv(envVar)) {
			ca.t.Skipf("Skipping Pygmalion test: missing required environment variable: %s", envVar)
		}
	}
...
func (ca *CharacterAssertion) AssetJannyAICookie() *CharacterAssertion {
	...
	for _, envVar := range requiredEnvVars {
		if stringsx.IsBlank(os.Getenv(envVar)) {
			ca.t.Skipf("Skipping JannyAI test: missing required environment variable: %s", envVar)
		}
	}
...
func (ca *CharacterAssertion) AssetJannyAIChromeConfig() *CharacterAssertion {
	if !router.IsChromeOK(func() impl.JannyChromeConfig {
		return impl.JannyChromeConfig{
			AutoFetchCookies: true,
		}
	}) {
		ca.t.Skip("Skipping JannyAI AutoFetch test: missing required chrome/chromium binary.")
	}
...
func (ca *CharacterAssertion) AssertNoErr() *CharacterAssertion {
	...
	require.Nil(ca.t, ca.err, "err should be nil, got: %v", ca.err)
