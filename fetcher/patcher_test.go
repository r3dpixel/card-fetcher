...
func TestPatchTags(t *testing.T) {
	...
	t.Run("should sort tags alphabetically by slug", func(t *testing.T) {
		...
		assert.Equal(t, "alpha", slugs[1])
		assert.Equal(t, "beta", slugs[2])
		assert.Equal(t, "charlie", slugs[3])
		assert.Equal(t, "zulu", slugs[4])
	})
	...
	t.Run("should handle unicode and special character tags", func(t *testing.T) {
		...
		assert.Len(t, sheet.Content.Tags, 5)
	...
	t.Run("should handle very large number of tags", func(t *testing.T) {
		...
		assert.Len(t, sheet.Content.Tags, 1001)
		assert.Len(t, metadata.Tags, 1001)
	})
...
func TestPatchBookName(t *testing.T) {
	...
	t.Run("name generation from blank names", func(t *testing.T) {
		...
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				book := &character.Book{Name: property.String("")}
...
// TestPatchLink tests the patchMetaFields function with complete coverage
